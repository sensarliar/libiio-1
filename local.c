	...
	struct timespec *start)
{
	...
	do {
		...
		ret = poll(pollfd, num_pfds, timeout_rel);
	} while (ret == -1 && errno == EINTR);

	if (num_pfds > 1 && (pollfd[1].revents & POLLIN))
		return -EBADF;
...
static int local_set_cancellable(const struct iio_device *dev,
	bool cancellable)
{
	struct iio_device_pdata *pdata = dev->pdata;

	pdata->cancellable = cancellable;
...
static const struct iio_backend_ops local_ops = {
	...
	.set_cancellable = local_set_cancellable,
	.cancel = local_cancel,
};
